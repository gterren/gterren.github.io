...
I constructed a sky imager (`Girasol Machine`) with far-infrared and multi-exposure fisheye visible light capabilities mounted on a solar tracker, so the Sun is in the center of the sky images throughout the day. We developed a noise reduction software and image fusion algorithm to merge the fisheye sky images with different exposures to obtain a low signal-to-noise ratio sky image with a high dynamic range {% cite TERRENSERRANO2021106914 %}. 
